Vagrant.configure("2") do |config|
  ...
  ['1', '2'].each do |client|
    #ip_addresses = "172.20.20.11,172.20.20.21"
    ip_prefix = client == '1' ? '172.20.20.12' : client == '2' ? '172.20.20.22' : '172.20.20.31'

    config.vm.define "consul-client-#{client}" do |cs|
      cs.vm.hostname = "consul-client-#{client}"
      cs.vm.network "private_network", ip: "#{ip_prefix}"
      cs.vm.provision "shell", path: "provision/scripts/init.hashi.client.sh", args: "#{client}", privileged: false
      cs.vm.provision "shell", path: "provision/scripts/install.dnsmasq.sh", privileged: false
